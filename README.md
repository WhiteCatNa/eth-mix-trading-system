...
| 奖励口径 | `r - λ·min(r,0)²`，r = desk 持仓窗口 PnL / 小时波动。GRPO 在每个再平衡点采 G 个动作、组内相对优势，没有 critic。曾用 differential Sharpe，详见 `nn/reward.py` |
...
strategy:
  ...
  decision: rl                          # 写入配置但不分支；只有 GRPO 或 flat
  ...
  grpo_group_size: 8                    # 每个再平衡点的候选动作数
  ppo_inner_epochs: 3                   # inner × replay = 每样本反传次数
