...
推理路径：最近 7 根 bar 的 [7, 30] 窗口 → median/IQR 稳健标准化 → 各成员
...
from betatrend.nn.dataset import FEATURE_NAMES, N_FEAT, SEQ_LEN, last_feature_window
...
class NeuralPolicy:
    ...
    def predict_unit(self, panel: pd.DataFrame) -> float:
        if not self._ready:
            return 0.0
        win = last_feature_window(panel, seq_len=self._seq_len)
