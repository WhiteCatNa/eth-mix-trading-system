...
impl OverlayEnv {
    pub fn from_bars(bars: Vec<Bar>, cfg: ResetCfg, flags: Option<[bool; 5]>) -> Self {
        ...
        let rm = RewardMachine::new(cfg.down_lambda, cfg.dd_inc, cfg.dd_level, cfg.clip);
        let mut env = Self {
            bars,
            cfg,
            x,
...
impl TradingEnv for OverlayEnv {
    ...
    fn reset(&mut self, cfg: ResetCfg) -> Vec<f32> {
        ...
        self.rm = RewardMachine::new(self.cfg.down_lambda, self.cfg.dd_inc, self.cfg.dd_level, self.cfg.clip);
        self.fit_scaler();
        self.rebuild_windows();
        self.exec.reset();
        self.rm.reset();
